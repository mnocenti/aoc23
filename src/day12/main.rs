...
fn part2(springs: &Springs) -> Result<usize> {
    Ok(springs.iter().map(|row| row.unfold()).map(|row |row.count_arrangements()).sum())
}
...
impl Row {
    ...
    fn arrangements(row : &[u8], current_group_length : usize, groups : &[usize]) -> usize {
        ...
        count += if [b'.',b'?'].contains(&row[0]) {
            let can_close_group = current_group_length == 0 || (!groups.is_empty() && current_group_length == groups[0]);
            if !can_close_group {
                // invalid arrangement
                0
